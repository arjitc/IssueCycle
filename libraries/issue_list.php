...
function general_issue_list($limit, $issue_status, $issue_status2=NULL) {
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$issue_project_name = get_project_name_from_project_id($row['issue_project_id']);
			...
			if(get_issue_status($id) == "Done") {
				echo "<span class='badge badge-success'>Done</span> $issue_subject";
			} else if(get_issue_status($id) == "In Progress") {
				echo "<span class='badge badge-info'>In Progress</span> $issue_subject";
			} else if(get_issue_status($id) == "On Hold") {
				echo "<span class='badge badge-dark'>On Hold</span> $issue_subject";
			} else {
				echo "<span class='badge badge-primary'>Open</span> $issue_subject";
			}
			echo "</h5>";
			echo "<small>$issue_open_since ago</small>";
			echo "</div>";
			echo "<span class='badge badge-pill badge-secondary'>$issue_project_name</span> ";
...
function assigned_issue_list($limit, $user_id, $issue_status, $issue_status2=NULL) {
	...
	if($issue_status2 != NULL) {
		$sql ="SELECT issue_members.issue_id, issues.issue_status, issues.issue_project_id FROM issue_members JOIN issues ON issue_members.issue_id = issues.id WHERE issue_members.issue_member_id='$user_id' AND issues.issue_status='$issue_status' OR issues.issue_status='$issue_status2' ORDER BY issue_members.issue_id LIMIT $limit";
	} else {
		$sql ="SELECT issue_members.issue_id, issues.issue_status, issues.issue_project_id FROM issue_members JOIN issues ON issue_members.issue_id = issues.id WHERE issue_members.issue_member_id='$user_id' AND issues.issue_status='$issue_status' ORDER BY issue_members.issue_id LIMIT $limit";
	}
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
		echo "<div class='list-group'>";
